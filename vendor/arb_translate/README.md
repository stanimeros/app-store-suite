...
4. (Optional) Select model used for translation. To do it add
   `arb-translate-model` to `l10n.yaml` or use command argument `--model`. The available options are `[gemini-3-flash-preview, gemini-3.1-pro-preview, gemini-3.5-flash, gemini-3.5-flash-lite, gemini-3.6-flash (default for Gemini), gpt-3.5-turbo, gpt-4, gpt-4-turbo, gpt-4o (default for OpenAI)]`
